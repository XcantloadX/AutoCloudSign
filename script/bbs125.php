...
class BBS125 extends Runner{
    public function run(string $aid, array $data){
        $cookie = $data["cookie"];
        $formhash = $this->getFormHash($cookie);
        $ret = newHttp("https://bbs.125.la/plugin.php?id=dsu_paulsign:sign&operation=qiandao&infloat=1")
        	->setCookie($cookie)
        	->post("formhash=$formhash&submit=1&targerurl=%2Fthread-14695011-1-1.html&todaysay=%E3%80%90%E9%A6%96%E5%8F%91%E3%80%91%E4%B8%80%E4%B8%AAAPI%E6%9C%89%E4%BA%BF%E4%B8%AA%E5%8A%9F%E8%83%BD%EF%BC%88NtQuerySystem+...&qdxq=kx")
        	->asJSON();
    ...
    private function getFormHash(string $cookie) : string{
    	$ret = newHttp("https://bbs.125.la/plugin.php?id=dsu_paulsign:sign")
    		->setCookie($cookie)
    		->get()
    		->asString();
