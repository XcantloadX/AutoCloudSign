...
class Bilibili extends Runner
{
    ...
    */
    private function signin($cookie)
    {
        ...
        //错误检查
        if ($json->code == -101) {
            $this->nb->append("Cookie 已失效！请重新设置。");
            logError("Cookie 已失效！请重新设置。");
            return;
        } elseif ($json->code != 0) {
            logError("未知错误！code=".$json->code);
            logError("返回 json: ".$ret);
            $this->nb->append("未知错误！code=".$json->code);
            $this->nb->append("返回 json: ".$ret, "%s", "返回 json:".PHP_EOL."```".PHP_EOL.$ret.PHP_EOL."```");
        }

        $name = $json->data->uname;
        $this->nb->append("@".$name, "%s", "### %s");


        $this->nb->append("硬币：".$this->getCoinNum($cookie));
        ...
        if ($ret->code == 1011040) {
            ...
            $this->nb->append("直播站签到失败".$ret->message);
        } else {
        ...
        if ($ret->msg == "clockin clockin is duplicate") {
            ...
            $this->nb->append("漫画站签到失败：".$ret->msg);
        }

        $this->nb->append("本月已签 ".$signedDays."/".$allDays." 天");
    }
